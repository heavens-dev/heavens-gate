...
from peewee import (BooleanField, CharField, CompositeKey, DateTimeField,
                    DeferredForeignKey, ForeignKeyField, IntegerField, Model)
...
class UserModel(BaseModel):
    ...
    remnawave_user_uuid = CharField(default=None, null=True)
    """UUID for the user in the Remnawave.
    Overrides the default UUID generated by `generate_deterministic_uuid_string()` if set."""

    organization_id = DeferredForeignKey(
        "OrganizationModel", backref="users", on_delete="SET NULL", null=True
    )
...
class OrganizationModel(BaseModel):
    ...
    subscription_type = CharField(default=None, null=True, choices=tuple(
        (subscription.value, subscription.name) for subscription in SubscriptionType
        if subscription is not SubscriptionType.ENTERPRISE)
    )
    """Type of subscription for every organization member. Currently unused. _Maaaaaaybe later_..."""
    registered_at = DateTimeField(default=datetime.datetime.now)
