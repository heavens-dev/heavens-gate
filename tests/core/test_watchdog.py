...
@pytest.fixture
def connection_events(db, wg_hub):
    return ConnectionEvents(wg_hub)
...
async def test_emit_connect(connection_events: ConnectionEvents):
    ...
    client.set_peer_status.assert_called_once_with(peer.id, PeerStatusChoices.STATUS_CONNECTED)
...
async def test_emit_disconnect(connection_events: ConnectionEvents):
    ...
    triggered_func.assert_called_once_with(client, peer)
    client.set_peer_status.assert_called_once_with(peer.id, PeerStatusChoices.STATUS_DISCONNECTED)
...
async def test_emit_timeout(connection_events: ConnectionEvents):
    ...
    triggered_func.assert_called_once_with(client, peer)
    client.set_peer_status.assert_called_once_with(peer.id, PeerStatusChoices.STATUS_TIME_EXPIRED)
